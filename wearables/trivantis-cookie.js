...
Description: 02-03-2015 Allowing more time to retrieve suspend_data before 
             executing the validation for suspend_data length.
...
var attDelaySetter,  //set this for adding delay while retrieving data;
	attDelayCounter = 0;  //set this for counting delay for second loop;
    ...
    if( this.bSCORM ) {
      ...
      if( this.name == 'cmi.core.total_time' || this.name == 'cmi.total_time' ) {
        ...
        //  var bErr = false;
		  ...
		  //bt988g: Refer ChkSuspendDataLen() method for comments.  All the code below the following line moved into the new method.
		  clearInterval(attDelaySetter);
		  attDelayCounter = 0;
		  attDelaySetter = setInterval(function(){ ChkSuspendDataLen(newData.length); }, 500);
        }
      }
    }
    else {
...
//BT988G:  Extracted some lines of the code from VarSave() method and added a custom check via setInterval method.
// The following is created due to an early retrieval of the data from LMS using GetSuspendData(); before data is being
// inserted through SetSuspendData().  Added an additional check after 200MS (1/5th of a second) to make sure GetSuspendData()
// either gets blank or some data from the system.

function ChkSuspendDataLen(l){
	var bErr = false;
//  alert(attDelayCounter);
	if(attDelayCounter == 2){
		clearInterval(attDelaySetter);

		var chkdata = String( GetSuspendData() )

		if( chkdata.length < parseInt(l) ) {
			bErr = true;
		}
